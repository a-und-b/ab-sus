...
export const Chatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Ho ho ho! 🎅 Ich bin Rudolph. Wie kann ich dir bei der Weihnachtsfeier helfen?' }
  ]);
  ...
  const getSystemInstruction = () => {
    const participants = dataService.getAll();
    const config = dataService.getConfig();
    const attending = participants.filter(p => p.status === 'attending');
    const foodList = attending.filter(p => p.food?.name).map(p => `- ${p.food?.name} (${p.food?.category})`).join('\n');
  ...
  useEffect(() => { if (isOpen && !chatSessionRef.current) initializeChat(); }, [isOpen]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      if (!chatSessionRef.current) await initializeChat();
      if (chatSessionRef.current) {
        const result = await chatSessionRef.current.sendMessage({ message: userMsg });
        setMessages(prev => [...prev, { role: 'model', text: result.text || "Mein Rentier-Navi spinnt." }]);
      }
    } catch {
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[90vw] sm:w-96 h-[500px] max-h-[80vh] bg-white rounded-3xl shadow-2xl flex flex-col z-50 border border-white overflow-hidden animate-fade-in">
          <div className="bg-xmas-red p-5 flex items-center gap-3 text-white">
            ...
            <div><h3 className="font-bold font-serif">Rudolph</h3><p className="text-xs text-red-100 opacity-80">Weihnachts-Bot</p></div>
          </div>

          <div className="flex-grow overflow-y-auto p-5 space-y-4 bg-stone-50 relative">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-stone-800 text-white rounded-br-none shadow-md' : 'bg-white text-stone-700 border border-stone-100 rounded-bl-none shadow-sm'}`}>
                  {msg.text}
                </div>
              </div>
            ))}
            {isLoading && <div className="flex justify-start"><div className="bg-white text-stone-400 p-3 rounded-2xl text-xs flex items-center gap-2"><Sparkles size={12} className="animate-spin" /> Tippt...</div></div>}
          ...
          <form onSubmit={handleFormSubmit} className="p-4 bg-white border-t border-stone-100 flex gap-2">
            <input
              type="text"
              value={input}
              ...
              autoFocus
            />
            <button type="submit" disabled={isLoading || !input.trim()} className="bg-xmas-red text-white p-3 rounded-xl hover:bg-red-700 disabled:opacity-50 transition-colors shadow-md shadow-red-200">
              <Send size={18} />
            </button>
          </form>
        </div>
