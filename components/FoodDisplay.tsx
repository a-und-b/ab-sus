  ...
  const groupedFood = Object.values(FoodCategory).map((category) => {
    ...
    const realItems = participants.filter(
      (p) =>
        p.status === 'attending' &&
        p.food &&
        p.food.category === category &&
        p.food.name.trim().length > 0
    );
              ...
              <ul className="space-y-2">
                ...
                {group.realItems.map((p) => {
                  ...
                  return (
                    <li
                      key={p.id}
                      className="group relative flex items-center gap-3 p-2 rounded-xl hover:bg-stone-50 transition-colors"
                    >
                      {/* Avatar / User */}
                      {showName ? (
                        <div className="relative shrink-0 group/tooltip cursor-help">
                          <Avatar
                            ...
                            image={p.avatarImage}
                            size={32}
                            className="bg-stone-100 ring-1 ring-stone-100"
                          />
                          {/* Tooltip */}
                      ...
                      {/* Dish Name */}
                      <span className="font-bold text-stone-800 text-sm flex-grow">
                        {p.food?.name}
                      </span>
                      ...
                      <div className="flex gap-1 shrink-0 opacity-80">
                        {p.food?.isVegan && (
                          <span title="Vegan" className="text-green-600 bg-green-50 p-1 rounded">
                            <Leaf size={12} />
                          </span>
                        )}
                        {p.food?.isGlutenFree && (
                          <span
                            title="Glutenfrei"
                            className="text-amber-600 bg-amber-50 p-1 rounded"
                          >
                            <WheatOff size={12} />
                          </span>
                        )}
                        {p.food?.isLactoseFree && (
                          <span
                            title="Laktosefrei"
                        ...
                        {p.food?.containsNuts && (
                          <span
                            title="Enthält Nüsse"
                            className="text-stone-400 bg-stone-100 p-1 rounded"
                          >
                            <Nut size={12} />
                          </span>
                        )}
